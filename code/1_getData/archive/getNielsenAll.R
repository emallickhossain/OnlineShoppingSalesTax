# Gets all products from Nielsen
library(data.table)
library(purrr)
library(stringr)
...
getPurchases <- function(year) {
  ...
  purchases <- fread(paste0(fileName, "Purchases/purchase", year, ".csv"),
                     select = c("trip_code_uc", "total_price_paid", "nonGroc"))
  purchases <- purchases[, .(full_price = sum(total_price_paid)), by = .(trip_code_uc, nonGroc)]
  purchases <- dcast(purchases, trip_code_uc ~ nonGroc, value.var = "full_price")
  setnames(purchases, c("trip_code_uc", "groc", "nonGroc"))
  purchases[is.na(nonGroc), "nonGroc" := 0]
  purchases[is.na(groc), "groc" := 0]
  purchases[, "total" := groc + nonGroc]
  ...
  trips[, "onlineSpending" := ifelse(retailer_code %in% 4700:4849, total, 0)]
  trips <- trips[, .(groc = sum(groc),
                     nonGroc = sum(nonGroc),
                     total = sum(total),
                     onlineSpending = sum(onlineSpending)),
                 by = .(household_code, panel_year, month)]
  trips[, "offlineSpending" := total - onlineSpending]
  return(trips)
}
